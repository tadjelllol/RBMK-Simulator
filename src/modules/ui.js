import { createElement, findConstructor, averageCalc } from './utils.js';
...
import { Fuel, Control, ControlAuto, Boiler, Moderator, Absorber, Reflector, Outgasser, Storage, Cooler, Heatex } from './column-types.js';
...
function initFuelPanel(rbmk, options) {
  ...
  fuels.forEach((fuel, index) => {
    ...
    const selfigniting = (test.selfRate > 0 || test.function === "SIGMOID") 
...
function initStatsPanel(rbmk, options) {
  ...
  const stats = [
    ...
    { id: "xenonPoisonLevel", title: "Xenon Poisoning", value: "0%", tooltip: "Average xenon poisoning of all fuel rods" },
    { id: "coreTempFuel", title: "Fuel Temp", value: "20°C", tooltip: "Average temperature of all fuel cores" },
    { id: "powerProduced", title: "Power Produced", value: "0 HE", tooltip: "Total heat energy produced by the reactor" },
...
function updateStats(rbmk, options) {
  ...
  document.querySelector('#powerProduced .stat-value').textContent = `${averageCalc(powerProducedd).toFixed(1)} HE`;
  ...
  const buttonActions = {
    "new": function(btn) {
      if (confirm("Create new RBMK or just remove the columns while keeping the width and height?")) {
        ...
        if (res === "rbmk") {
          // Ask for dimensions
          let width = parseInt(prompt("RBMK width") || "15", 10);
          let height = parseInt(prompt("RBMK height") || "15", 10);
  ...
  document.addEventListener('click', (e) => {
    ...
    if (x >= 0 && x <= maxX && y >= 0 && y <= maxY) {
      ...
      if (!options.simulating && options.place.placing) {
        ...
        rbmk.columns[index] = getConstructor(options.place.selected);
        
        if (options.place.selected !== "nothing" && rbmk.columns[index]) {
    ...
    if (mPos[0] >= toolsRect.x && mPos[0] <= toolsRect.right && 
        mPos[1] >= toolsRect.y && mPos[1] <= toolsRect.bottom && 
        !options.simulating && options.place.placing) {
...
function getConstructor(block) {
  switch (block) {
    case "blank": return new Column(0, {enabled: true, text: "<b>BLANK</b>"});
    case "fuel": return new Fuel();
    case "control": return new Control();
    case "control_auto": return new ControlAuto();
    case "boiler": return new Boiler();
    case "moderator": return new Moderator();
    case "absorber": return new Absorber();
    case "reflector": return new Reflector();
    case "outgasser": return new Outgasser();
    case "storage": return new Storage();
    case "cooler": return new Cooler();
    case "heatex": return new Heatex();
  ...
  setInterval(() => {
    ...
    if (selectedIndex !== options.config.prevColumn) {
      ...
      if (selectedIndex >= 0 && selectedIndex < rbmk.columns.length && rbmk.columns[selectedIndex] !== null) {
        ...
        const columnName = createElement('p', {
          className: 'noMargin',
          style: { fontSize: '35px' }
...
export function setupConfigMenuAction(rbmk, options) {
  window.configMenuAction = function() {
    const btn = document.elementFromPoint(mPos[0], mPos[1]);
    ...
    switch (btn.getAttribute("configmenuaction")) {
      ...
      case "cauto_levelUpper":
        ...
        let upperNum = parseInt(upperRes, 10);
      ...
      case "cauto_levelLower":
        ...
        let lowerNum = parseInt(lowerRes, 10);
      ...
      case "cauto_heatUpper":
        ...
        let heatUpperNum = parseInt(heatUpperRes, 10);
      ...
      case "cauto_heatLower":
        ...
        let heatLowerNum = parseInt(heatLowerRes, 10);
  ...
  window.fuelClick = function() {
    const btn = document.elementFromPoint(mPos[0], mPos[1]);
    const col = rbmk.columns[options.config.columnIndexSelected];
    
    if (col instanceof Fuel) {
      const fuelIndex = parseInt(btn.getAttribute("data-fuelname") || btn.getAttribute("fuelname"), 10);
