...
export class Fuel extends Column {
   ...
   */
  getConfig() {
    const selfigniting = (this.fuel.selfRate > 0 || this.fuel.function === "SIGMOID") 
    ...
    const moderationBtn = createElement('button', {
...
export class ControlAuto extends Control {
  ...
  getConfig() {
    ...
    const interpolationBtn = createElement('button', {
      className: "textButton",
      style: { fontSize: "27px" },
    ...
    const levelUpperBtn = createElement('button', {
      className: "textButton",
      style: { fontSize: "27px" },
    ...
    const levelLowerBtn = createElement('button', {
      className: "textButton",
      style: { fontSize: "27px" },
    ...
    // Max heat button
    const heatUpperBtn = createElement('button', {
      className: "textButton",
      style: { fontSize: "27px" },
    ...
    // Min heat button
    const heatLowerBtn = createElement('button', {
      className: "textButton",
      style: { fontSize: "27px" },
...
export class Boiler extends Column {
  ...
  update(ticks, rbmk) {
    ...
    // Boiling
    const heatCap = this.getHeatFromSteam();
    const heatProvided = this.heat - heatCap;
    ...
    this.steam -= window.options.rbmkStuff.boilerOutputRate;
    this.steam2 += window.options.rbmkStuff.boilerOutputRate;
    ...
    this.producedPower = 0;
    if (window.options.rbmkStuff.boilerOutputRate > 0) {
    ...
    // Condensation
    const convert = Math.min(this.spentSteam, this.feedwaterMax - this.feedwater);
  ...
  getTraitFromSteam() {
    ...
    return {amountReq: 100, amountProduced: 1, efficiency: 1, heatEnergy: 200};
  }
  ...
  getConfig() {
    ...
    const compressionBtn = createElement('button', {
      className: "textButton",
      style: { fontSize: "27px" },
      configmenuaction: "compression",
      onclick: "configMenuAction()"
    }, `Steam compression: ${this.compressors[this.steamType]}`);
